...
exports.is_kthlan = (req) => {
  ...
  const split_ip = (ip) => {
    ...
    return (
      parseInt(ip_parts[0]) * Math.pow(2, 24) +
      parseInt(ip_parts[1]) * Math.pow(2, 16) +
      parseInt(ip_parts[2]) * Math.pow(2, 8) +
      parseInt(ip_parts[3])
    );
  };
  ...
  for (const network of config.kthlan) {
    ...
    var net_int =
      split_ip(split[0]) & (~(Math.pow(2, net_bit_count) - 1) & 0xffffffff);
...
exports.setConnection = (c) => {
  ...
  Queue.belongsToMany(Room, {
    as: "Rooms",
    through: "queues_rooms",
    foreignKey: "queue_id",
  });
  Room.belongsToMany(Queue, {
    as: "Queues",
    through: "queues_rooms",
    foreignKey: "room_id",
  });
  ...
  Queue.belongsToMany(Profile, {
    as: "Assistants",
    through: "queues_assistants",
    foreignKey: "assistant_id",
  });
  Profile.belongsToMany(Queue, {
    as: "AssistantInQueues",
    through: "queues_assistants",
    foreignKey: "queue_id",
  });
  ...
  Booking.belongsToMany(Profile, {
    as: "BookingStudents",
    through: "bookings_students",
    foreignKey: "booking_id",
  });
  Profile.belongsToMany(Booking, {
    as: "StudentInBookings",
    through: "bookings_students",
    foreignKey: "student_id",
  });
  ...
  QueueQueuing.hasMany(QueueQueuingHandler, {
    as: "handlers",
    foreignKey: "queue_student_id",
  });
  QueueQueuingHandler.belongsTo(QueueQueuing, {
    foreignKey: "queue_student_id",
  });
...
exports.task_update = (task) => {
  ...
  if (duration <= 0) {
    ...
    timeouts[task.id] = setTimeoutAt(
      exports.task_timeout,
      task.deadline,
      task.id
    );
  }
...
exports.create_token = (profile_id) =>
  Token.create({
    token: crypto.randomBytes(32).toString("hex"),
    profile_id: profile_id,
  });

exports.get_profiles = (raw_list) =>
  new Promise((resolve, reject) => {
    ...
    for (const raw_item of raw_list) {
      if (
        ...
        (raw_item.hasOwnProperty("id") && raw_item.hasOwnProperty("user_name"))
      ) {
        resolve(null);
        return;
      }
...
exports.get_profile_by_user_name = (user_name) =>
  new Promise((resolve, reject) => {
...
exports.save_event = (queue_id, user_id, type, data) =>
  Event.create({
    queue_id: queue_id,
    user_id: user_id,
    type: type,
    ...
    Room.findAll({ order: [["name", "ASC"]] }).then((rooms) => {
      Computer.findAll().then((computers) => {
        ...
        for (const room of rooms) {
          const computers_in_room = computers
...
exports.get_booking = (queue, booking_id) =>
  Booking.findOne({
    where: {
      id: booking_id,
      queue_id: queue.id,
...
exports.delete_booking = (booking) =>
  new Promise((resolve, reject) => {
...
exports.get_action_by_name = (queue, name) =>
  Action.findOne({ where: { queue_id: queue.id, name: name } });
...
exports.queue_handling_assistant_add = (queue_queuing, user_id) =>
  QueueQueuingHandler.findOrCreate({
    where: {
...
exports.queue_handling_assistant_remove = (queue_queuing, user_id) =>
  QueueQueuingHandler.update(
    {
      timestamp_leave: Date.now(),
    },
    {
...
exports.add_room_to_queue = (room, queue) =>
  new Promise((resolve, reject) => {
...
exports.remove_room_from_queue = (room, queue) =>
  new Promise((resolve, reject) => {
...
exports.add_student_to_queue = (student, queue) =>
  new Promise((resolve, reject) => {
...
exports.remove_student_from_queue = (student, queue) =>
  new Promise((resolve, reject) => {
...
exports.add_assistants_to_queue = (assistants, queue) =>
  new Promise((resolve, reject) => {
...
exports.remove_assistant_from_queue = (assistant, queue) =>
  new Promise((resolve, reject) => {
...
exports.get_task = (task_id) =>
  Task.findOne({ where: { id: task_id }, include: [{ model: Queue }] });

exports.add_task_to_queue = (queue, type, data, deadline) =>
  new Promise((resolve, reject) => {
    Task.create({
      queue_id: queue.id,
      type: type,
...
exports.remove_expired_bookings = () => {
  ...
  Booking.findAll({
    ...
    if (smallest_not_expired !== null) {
      booking_timer = setTimeoutAt(
        exports.remove_expired_bookings,
        smallest_not_expired
      );
    } else {
      booking_timer = null;
    }
...
exports.get_queue_events = async (queue, timestamp_start, timestamp_end) =>
  Event.findAll({
    where: {
      queue_id: queue.id,
      timestamp: {
...
exports.get_queue_queuings = async (queue, timestamp_start, timestamp_end) =>
  QueueQueuing.findAll({
    where: {
      queue_id: queue.id,
      [Sequelize.Op.or]: [
        {
...
exports.nice_booking = (booking) => {
  const vinfo = this.get_booking_vinfo(booking);
...
exports.get_queue_info = (req, res, queue) => {
  if (queue === null) {
    res.status(404);
    res.end();
    return;
    ...
    model.get_actions(queue).then((actions) => {
      model
        ...
        .then(async (has_permission) => {
          ...
          bookings = bookings.map((b) => this.nice_booking(b));
...
exports.io_emit_update_queuing = (queue) =>
  exports.io_emit_update_queue(queue, { queuing: queuing[queue.id] });
...
exports.io_emit_update_queue = (queue, changes) => {
  ...
  io.sockets.sockets.forEach((socket) => {
    socket.emit(
      "update_queue",
      socket.handshake.session.hasOwnProperty("profile")
        ? message_user
        : message_guest
    );
  });
...
exports.io_emit_to_assistants = (queue, key, message) => {
  queue.getAssistants().then((assistants) => {
    ...
    io.sockets.sockets.forEach((socket) => {
      if (
        socket.handshake.session.hasOwnProperty("profile") &&
        (socket.handshake.session.profile.teacher ||
          ids.includes(socket.handshake.session.profile.id))
      ) {
