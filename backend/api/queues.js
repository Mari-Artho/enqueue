...
router.get("/:name", (req, res) => {
  model.get_queue(req.params.name).then((queue) => {
    model.get_queue_info(req, res, queue);
  });
});
...
router.get("/:name/bookings", (req, res) => {
  model.get_queue(req.params.name).then((queue) => {
    ...
    model.get_bookings(queue).then((bookings) => {
      bookings = bookings.map((b) => model.nice_booking(b));
      ...
      model
        ...
        .then((has_permission) => {
          ...
          model
            ...
            .then((booking) => {
              model.io_emit_update_booking(queue, model.nice_booking(booking));

              res.status(201);
              res.json(model.nice_booking(booking));
            });
        });
    });
  });
